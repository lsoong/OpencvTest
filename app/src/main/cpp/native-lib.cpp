...
extern "C" {
    ...
    Java_com_example_hihi4_opencvtest4_MainActivity_detect(JNIEnv *, jobject,
    ...
    Java_com_example_hihi4_opencvtest4_MainActivity_loadCascade(JNIEnv *env, jobject,
        ...
        if (((CascadeClassifier *) ret)->empty()) {
            __android_log_print(ANDROID_LOG_DEBUG, "native-lib :: ", "CascadeClassifier로 로딩 실패 %s",
                                nativeFileNameString);
        } else {
            __android_log_print(ANDROID_LOG_DEBUG, "native-lib :: ", "CascadeClassifier로 로딩 성공 %s",
                                nativeFileNameString);
            return ret;
        }
    }
